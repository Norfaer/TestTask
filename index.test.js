...
describe(`Coin service test group:`, () => {
  ...
  var genUniqueInts = gen.uniqueArray(gen.intWithin(2, 100), (int) => int, { minSize: 5, maxSize: 10 });
  check.it(`Run test for ${coinService.testCount} times`, { times: coinService.testCount }, gen.intWithin(coinService.minValue, coinService.maxValue), genUniqueInts, (n, a) => {
    // Make sure we have coin "1" in array;
    a.push(1);
    let result;
    let resultMin;
    try {
      result = lib.doCoinsExchange(n, a);
      resultMin = lib.doCoinsExchangeMin(n, a);
    } catch (e) {
      assert.fail("Should never get here");
    }
    console.log(`Test case ${n} (greedy) => ${a.join(",")} => ${result.join(",")} (count: ${result.length})`);
    console.log(`Test case ${n} (min)    => ${a.join(",")} => ${resultMin.join(",")} (count: ${resultMin.length})`);
    ...
    checkSumMin = resultMin.reduce((acc, cur) => acc + cur, 0);
    // Also we check that returned array is within [1,n] size range
    return n === checkSum && checkSumMin === n && result.length >= 1 && result.length <= n && resultMin.length <= result.length;
  });
});
